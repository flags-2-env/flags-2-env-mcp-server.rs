//! Immutable flags-2-env identity and provider-operation contract.
...
mod tests {
    ...
    fn scope_and_provider_contract_are_exact() {
        let spec = org_spec();
        assert_eq!(spec.organization, "flags-2-env");
        assert_eq!(spec.repository, "flags-2-env/flags-2-env-mcp-server.rs");
