...
The exact `flags-2-env/flags-2-env-mcp-server.rs` organization, repository, service,
...
- `github_posture` — the exact flags-2-env repository and CI posture;
...
- `supabase_posture` — the configured flags-2-env Supabase health boundary;
- `neon_posture` — exact Neon organization/project and branch metadata;
- `cloudflare_posture` — the configured flags-2-env zone and DNS posture;
- `k8s_posture` — the exact flags-2-env namespace in the
  `ORESoftware/k8s-cluster` deployment plane;
- `nats_posture` — the flags-2-env service and dependency subjects; and
...
local stdio process accepts no bearer token. The `flags-2-env-mcp-http` binary
...
public resource as audience, requires the flags-2-env project realm, AAL2,
`mcp:read` plus `flags-2-env:inspect`, and a `flags-2-env_viewer` or
`flags-2-env_operator` role. It never forwards a caller token upstream. Exact
...
- Kubernetes: `ORE_MCP_K8S_ENABLED=1`, optionally
  `ORE_MCP_K8S_NAMESPACE` (otherwise the exact `flags-2-env` namespace); and
- NATS: `ORE_MCP_NATS_URL`, scoped to
  `flags-2-env.mcp.service.read.v1` and `flags-2-env.mcp.dependencies.read.v1`.
